...
# Streamlit app
st.title("VISH - Versatile Interpretive Syntax Handler")

seed_text = st.text_input("Enter seed text:", "Once upon a time")
num_words = st.slider("Number of words to generate:", 1, 100, 50)
temperature = st.slider("Temperature:", 0.1, 2.0, 1.0)
top_p = st.slider("Top-p (nucleus) sampling:", 0.1, 1.0, 0.9)

if st.button("Generate Text"):
    generated_text = generate_text(model, tokenizer, seed_text, max_length, num_words, device, temperature, top_p)
